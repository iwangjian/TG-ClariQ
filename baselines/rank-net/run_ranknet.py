...
def run_train(args):
    ...
    save_path = "%s/ranknet.model" % args.log_dir
    # -ranker <type>	Specify which ranking algorithm to use
 	...
 	#	7: ListNet
 	#	8: Random Forests
    java_cmd = "java -jar {} -train {} -validate {} -ranker 1 -metric2t MAP -save {} -epoch {} -layer {} -node {} -lr {}".format(
        jar_path, train_path, dev_path, save_path, args.num_epochs, args.num_layers, args.num_nodes, args.lr)
...
def run_test(args):
    ...
    jar_dir = os.path.dirname(os.path.realpath(__file__))
    jar_path = os.path.join(jar_dir, "RankLib-2.15.jar")
    model_path = "%s/ranknet.model" % args.log_dir
