...
def evaluate_ranking(preds, labels):
    qrels = {}
    qrels['model'] = {}
...
def evaluate_and_aggregate(preds, labels, metrics):
    ...
    Returns: dict with the METRIC results per model and query.
    """
    results = evaluate_ranking(preds, labels)
...
def evaluate(preds, labels, metric="acc"):
    """
    preds: list of lists, shaped (batch_size, vocab_size)
    labels: list, shaped (batch_size)
    """
    acc = 0
